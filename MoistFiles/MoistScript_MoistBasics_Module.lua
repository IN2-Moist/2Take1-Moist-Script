...
M_Func["VersionWebCheck"] = function()
	...
	if versiondata[1][1] == MoistScript_NextGen then
		...
		TextPrintToScreen("~b~~h~MoistScript NG ~g~~h~You Have the Latest!", "~y~~h~Version: " .. tostring(versiondata[1][1]) .." ~w~Enjoy!", 3000, 000)
	elseif versiondata[1][1] ~= MoistScript_NextGen then
		...
		TextPrintToScreen("~b~~h~MoistScript NG ~r~~h~Version is Outdated!", "~y~~h~Version: " .. tostring(versiondata[1][1]) .." is Released", 3000, 000)
		system.yield(3000)
...
M_Func["Blip_Flash"] = function(BlipID, delay)
	...
	--	GTA_Natives.SET_BLIP_FLASHES_ALTERNATE(PlayerBlip, true)
	system.yield(delay)
...
M_Func["TriggerEvent2"] = function(event, pid, ...)
	...
	if player.is_player_valid(pid) and not Session_Players[pid].Exclude then
		system.yield(25)
		if MoistSafeMode == true and pid ~= player.player_id() then
			script.trigger_script_event_2(1 << pid, event, ...)
			system.yield(10)
		elseif MoistSafeMode ~= true and pid == player.player_id() then
			script.trigger_script_event_2(1 << pid, event, ...)
			system.yield(10)
		end
	end
