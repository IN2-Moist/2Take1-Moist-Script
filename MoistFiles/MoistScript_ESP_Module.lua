...
Features.ESP_Parent = menu.add_feature("ESP Options", "parent", LocalFeatures.LocalSettings.id)
...
local Object_BOX_ESP = menu.add_feature("Box Object ESP", "toggle", Features.ESP_Parent.id, function(feat)
	if type(feat) == "number" then
		return
	end
	while feat.on do
...
local Player_ESP_Box = menu.add_feature("Player_ESP_Box Distance:", "value_f", Features.ESP_Parent.id, function(feat)
	if type(feat) == "number" then
		return
	end
	while feat.on do
